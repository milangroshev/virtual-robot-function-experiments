...
Navigates THREE robots simultaneously in loops through 5 waypoints each:
  ...
  Robot 3 (panther3): BOTTOM_RIGHT → BOTTOM_LEFT → TOP_LEFT → MIDDLE → TOP_RIGHT → BOTTOM_RIGHT

Usage:
  python3 three_robot_patrol.py [num_loops] [warmup_secs]
...
def main():
    ...
    # Create all three robot nodes with unique names
    ...
    for i, robot in enumerate(all_robots, 1):
        robot1.get_logger().info(
            f'Waiting for Robot {i} ({robot.namespace}) action server...'
        )
        if not robot.wait_for_server():
            robot1.get_logger().error(
                f'Aborting: Robot {i} ({robot.namespace}) '
                f'action server not found'
            )
            for r in all_robots:
                r.cleanup()
            rclpy.shutdown()
    ...
    # --- Warmup ---
    robot1.get_logger().info(
        f'All three robots ready. Warmup pause: {warmup_secs}s '
    ...
    robot1.get_logger().info(
        f'Starting three-robot patrol ({loop_label} per robot)'
    )

    patrol_configs = [
        (robot1, PATROL_ORDER_ROBOT1),
        (robot2, PATROL_ORDER_ROBOT2),
        (robot3, PATROL_ORDER_ROBOT3),
    ]

    threads = []
    for robot, order in patrol_configs:
        t = threading.Thread(
            target=robot.run_patrol,
    ...
    # --- Summary ---
    total_goals = sum(r.total_goals for r in all_robots)
    total_success = sum(r.total_successes for r in all_robots)
    total_fail = sum(r.total_failures for r in all_robots)
    rate = total_success / total_goals * 100 if total_goals > 0 else 0

    robot1.get_logger().info('')
    robot1.get_logger().info('══════════ THREE-ROBOT PATROL SUMMARY ══════════')
    for i, robot in enumerate(all_robots, 1):
        r_rate = (
            robot.total_successes / robot.total_goals * 100
            if robot.total_goals > 0 else 0
        )
        robot1.get_logger().info(
            f'  Robot {i} ({robot.namespace:>9s}): {robot.loop_count} loops, '
            f'{robot.total_successes}/{robot.total_goals} goals '
            f'({r_rate:.0f}%)'
        )
    robot1.get_logger().info(
